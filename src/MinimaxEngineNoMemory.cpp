...
double MinimaxEngineNoMemory2::minimax(const Position& pos, double alpha, double beta, int depth) {
...
std::string MinimaxEngineNoMemory2::move(Position pos) {
    std::string bestMove = pos.moves()[0].lastMoveString;
    double bestValue = -inf;
    int minSign = pos.mover == WHITE_MOVE ? 1 : -1;
    for (const auto &move : pos.moves()) {
...
[[noreturn]] void MinimaxEngineNoMemory2::infinite(const Position &pos, int maxOutput) {
...
std::vector<std::pair<Position, double>> MinimaxEngineNoMemory2::sortedMovesWithScores(const Position &pos) const {
    bool descending = pos.mover == WHITE_MOVE;
    std::vector<std::pair<Position, double>> moves;
    for (const Position& move : pos.moves()) {
...
std::vector<Position> MinimaxEngineNoMemory2::bestSequence(const Position &start, int depth) {
    auto seq = _bestSequence(start, depth);
    std::reverse(seq.begin(), seq.end());
    return seq;
}

std::vector<Position> MinimaxEngineNoMemory2::_bestSequence(const Position &start, int depth) {
    if (depth == 0) {
        return {};
    }
